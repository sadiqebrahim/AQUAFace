'''
Evaluation on LFW, CFP_FP, AgeDB, CPLFW and CALFW.
...
from config.config_eval import configurations
from models.model_resnet import ResNet_50, ResNet_101, ResNet_152
from models.model_irse import IR_50, IR_101, IR_152, IR_SE_50, IR_SE_101, IR_SE_152
from models.model_mobilefacenet import MobileFaceNet
from head.metrics import ArcFace, CurricularFace
from loss.focal import FocalLoss
from utils.utils import get_val_data, perform_val, get_time, AverageMeter, accuracy
...
if __name__ == '__main__':
    ...
    BACKBONE = resnet_face18(use_se=False)  # Replace with your model
    checkpoint = torch.load('iresnet18_casia_dp.pth') # Replace with your path

    mystatedict = {}
    for key in checkpoint.keys():
        m = key.replace("module.","")
        mystatedict[m] = checkpoint[key]
    BACKBONE.load_state_dict(mystatedict, strict=False)
    ...
    print("Thres: LFW Acc: {}, CFP_FP Acc: {}, AgeDB Acc: {}, CPLFW Acc: {} CALFW Acc: {}".format(best_threshold_lfw, best_threshold_cfp_fp, best_threshold_agedb, best_threshold_cplfw, best_threshold_calfw))
